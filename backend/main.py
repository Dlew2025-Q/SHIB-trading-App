...
This is a very common issue with API keys copied from web interfaces, and re-entering it perfectly is usually the solution.
```python
# backend/main.py
...
async def ai_trade_signal(request_body: dict):
    ...
    # FIX: Ensure prompt is a single string literal, not concatenated with implicit newlines
    prompt = "\n".join(prompt_parts)
